...
const integrationList = computed(() => {
  const data = getters['integrations/getAppIntegrations'].value;
  const filteredData = data.filter(integration =>
    ['ecommerce'].includes(integration.type)
  );
  return filteredData;
});
  ...
  >
    <template #header>
      <BaseSettingsHeader
        ...
        :description="$t('CHATBOT_SETTINGS.INTEGRATION.DESCRIPTION')"
